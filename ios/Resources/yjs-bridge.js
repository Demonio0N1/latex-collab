...
${err.toString()}`);
  ...
  var nextId = 1;
  function room(id2) {
    const r = rooms.get(id2);
    if (!r) throw new Error("unknown room " + id2);
    return r;
  }
  globalThis.YBridge = {
    ...
    create() {
      ...
      doc2.on("update", (update, origin) => {
        ...
        outgoing.push(u8ToB64(toUint8Array(enc)));
      });
      ...
      rooms.set(id2, { doc: doc2, awareness, text: text2, outgoing });
      return id2;
    },
    destroy(id2) {
      const r = rooms.get(id2);
    ...
    syncStep1(id2) {
      const { doc: doc2 } = room(id2);
      const enc = createEncoder();
      writeVarUint(enc, MESSAGE_SYNC);
      writeSyncStep1(enc, doc2);
      return u8ToB64(toUint8Array(enc));
    },
    // Feed an incoming binary frame (base64). Returns a base64 reply to send
    // back, or "" if none.
    receive(id2, b64) {
      const { doc: doc2, awareness } = room(id2);
      const dec = createDecoder(b64ToU8(b64));
      const type = readVarUint(dec);
      if (type === MESSAGE_SYNC) {
        const enc = createEncoder();
        writeVarUint(enc, MESSAGE_SYNC);
        readSyncMessage(dec, enc, doc2, REMOTE);
        if (length(enc) > 1) return u8ToB64(toUint8Array(enc));
        return "";
      }
      if (type === MESSAGE_AWARENESS) {
        applyAwarenessUpdate(awareness, readVarUint8Array(dec), REMOTE);
        return "";
      }
      return "";
    },
    ...
    setText(id2, next) {
      const { doc: doc2, text: text2 } = room(id2);
      const prev = text2.toString();
      if (prev === next) return;
      let start = 0;
      ...
      doc2.transact(() => {
        if (endPrev > start) text2.delete(start, endPrev - start);
        if (endNext > start) text2.insert(start, next.slice(start, endNext));
      }, LOCAL);
    },
    // Presence: publish this user's identity/cursor for the awareness protocol.
    setLocalUser(id2, name, color) {
      room(id2).awareness.setLocalStateField("user", { name, color });
    },
    // Peers currently connected (excluding entries with no user), as JSON.
    peers(id2) {
      const states = room(id2).awareness.getStates();
      const localId = room(id2).awareness.clientID;
      const list = [];
